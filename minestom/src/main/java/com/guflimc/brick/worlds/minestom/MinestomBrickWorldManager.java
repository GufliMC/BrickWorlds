...
public class MinestomBrickWorldManager extends AbstractWorldManager implements MinestomWorldManager {
    ...
    @Override
    public Optional<World> worldByName(@NotNull String name) {
        return worlds().stream().filter(world -> world.info().name().equalsIgnoreCase(name)).findFirst();
    }

    @Override
    public Collection<WorldInfo> loadedWorlds() {
        return worlds().stream().map(World::info).collect(Collectors.toList());
    }

    @Override
    public CompletableFuture<Void> saveAll() {
        return CompletableFuture.allOf(worlds().stream().map(World::save)
                .toArray(CompletableFuture[]::new));
    }

    @Override
    public CompletableFuture<Void> saveWorld(@NotNull String name) {
        return worldByName(name).map(World::save)
    ...
    @Override
    public CompletableFuture<Void> createWorld(@NotNull String name) {
        return createWorld(name, "void");
    }

    @Override
    public CompletableFuture<Void> createWorld(@NotNull String name, String generatorId, String[] generatorArgs) {
    ...
    private CompletableFuture<Void> loadWorld(@NotNull File directory) {
        if (!directory.exists()) {
    ...
    @Override
    public CompletableFuture<Void> loadWorld(@NotNull String name) {
